...
from PTengine.models import Article, Response
...
def save_mention(mention, source_info, target_url, target_obj):
	...
	mention.content = source_info.snippetWithLink(target_url)
	mention.response_to=target_obj
	mention.original_title=source_info.title()
...
def post_mention(accept_header, source, target):
	## REMOVE ?PARAMETERS AND #REFS
	## WHAT PARTS ACTUALLY NEED HTML OR JSON IDENTIFIER?
	## also convert source target to absolute url's  just in case?
